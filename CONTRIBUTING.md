...
Or `make check PYTHON=python3.12` to select an interpreter. The suite checks syntax, Markdown links and anchors, configuration validation, catalog consistency, CLI behavior, reproducible builds, release integrity, the rendered `/help` page, deployment locking, rollback, Stash authentication and client lifecycle behavior. It needs local Unix sockets and loopback HTTPS but no personal configuration or root access. Tests use temporary homes, synthetic keys, an isolated agent and a temporary certificate; they never load real credentials, modify shell profiles or inspect personal clients. CI runs Python 3.9 and 3.12.
...
| Codex | Native/npm coexistence, reuse, catalog/auth compatibility, prompt and `-t`/`-u` paths, failed-write recovery, uninstall with retained history and reinstall |
